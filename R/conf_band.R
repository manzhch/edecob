...
ratio_in_ci <- function(alpha_p,
                        bt_S,
                        smoother_pts, # bt_S/bt_tot_rep and smoother_pts should have the same size
                        bt_tot_rep,
                        alpha = 0.05) {
  ...
  bt_S$init_est <- bt_S - rep(smoother_pts, bt_tot_rep)

  # calculate quantiles
  for (ii in 1:length(uniq_date)) {
    my_quantile_lower[ii] <-
      stats::quantile(bt_S$init_est[bt_S$date == uniq_date[[ii]]], alpha_p)
    my_quantile_upper[ii] <-
      stats::quantile(bt_S$init_est[bt_S$date == uniq_date[[ii]]],
               1 - alpha_p)
  }
  ...
  bt_S$in_interval <-
    as.logical(pmin(bt_S$init_est <= rep(my_quantile_upper, bt_tot_rep),
...
# are within CI
find_ptw_alpha <- function(bt_S,
                           smoother_pts,
                           bt_tot_rep,
                           alpha){

  if (nrow(bt_S) > bt_tot_rep &&
      nrow(bt_S[!is.na(bt_S$value), ]) > 0 &&
      sign(ratio_in_ci(0, bt_S, smoother_pts, bt_tot_rep, alpha)) !=
      sign(ratio_in_ci(0.05, bt_S, smoother_pts, bt_tot_rep, alpha))) {

    return(stats::uniroot(ratio_in_ci, lower = 0, upper = 0.05,
                          bt_S, smoother_pts, date, bt_tot_rep, alpha))

  } else if (nrow(bt_S) > bt_tot_rep &&
             nrow(bt_S[!is.na(bt_S$value), ]) > 0 &&
             sign(ratio_in_ci(0, bt_S, smoother_pts, bt_tot_rep, alpha)) ==
             sign(ratio_in_ci(0.05, bt_S, smoother_pts, bt_tot_rep, alpha))) {
    # print(2)
...
conf_band <- function(bt_S,
                      smoother_pts,
                      bt_tot_rep,
                      alpha){

  #bt_S$date <- as.Date(unlist(bt_S$date), format = "%Y-%m-%d", origin = "1970-01-01")

  # calculate pointwise quantile
  dates <- sort(unlist(unique(bt_S$date)))
  my_quantile_lower <- numeric(length(dates))
  my_quantile_upper <- numeric(length(dates))

  ptw_alpha <- find_ptw_alpha(bt_S, smoother_pts, bt_tot_rep, alpha)

  # calculate quantiles
  if (nrow(bt_S) > 0) {
    both_quantiles <-
      sapply(split(bt_S, factor(unlist(bt_S$date))),
             function(x){
               if (sum(!is.na(x)) > 0 &&
                   !is.na(ptw_alpha) &&
  ...
  for (ii in unique(as.character(sort(smoother_pts$date[
    smoother_pts$date %in% unique(bt_S$date)])))) {
